{**
 * plugins/paymethod/emspubpaddle/templates/paymentTypesForm.tpl
 *
 * Copyright (c) 2014-2021 Simon Fraser University
 * Copyright (c) 2003-2021 John Willinsky
 * Distributed under the GNU GPL v3. For full terms see the file docs/COPYING.
 *
 * Payment type form (Overridden for Paddle APC Product ID).
 *}
<script>
	$(function() {ldelim}
...
<form class="pkp_form" id="paymentTypesForm" method="post" action="{url op="savePaymentTypes"}">
	...
	{fbvFormArea id="authorFeesArea" title="manager.payment.authorFees"}
		...
		<div class="fbvFormSection">
			{fbvElement 
				type="text" 
				name="paddleApcProductId" 
				id="paddleApcProductId" 
				label="plugins.paymethod.emspubpaddle.settings.apcProductId" 
				value=$paddleApcProductId 
				size=$fbvStyles.size.MEDIUM
			}
			<p class="pkp_help">{translate key="plugins.paymethod.emspubpaddle.settings.apcProductId.description"}</p>
		</div>
	{/fbvFormArea}
